...
class VcfToCsvConverter:
    ...
    def __output(self, text):
        ...
        if self.quote == True:
            text = text.replace('\\n', '\n').replace('\\r', '\r').replace(
                '\\'+self.delimiter, self.delimiter).strip()
        else:
            text = text.strip()
    ...
    def __resetRow(self):
        ...
        self.telephoneCount = {'HOME PHONE': 1, 'WORK PHONE': 1,
                               'MOBILE PHONE': 1, 'HOME FAX': 1, 'WORK FAX': 1}
    ...
    def __getfileNames(self):
        try:
            if os.path.isdir(self.inputPath):
                self.inputFileArray = glob.glob(
                    os.path.join(self.inputPath, '*.vc[sf]'))
            else:
    ...
    def __processLine(self, pieces):
        ...
        self.__trace("item match: %s " % re.match(
            'item.*', pre[0].split(".")[0], re.I))
    ...
    def __processEmail(self, pre, p):
        ...
        if self.emailCount[hwm] <= self.maxEmails:
            ...
            self.__trace("__email %s %s: %s" %
                         (hwm, self.emailCount[hwm], p[0]))
            self.emailCount[hwm] += 1
        else:
            self.data['WARNING'] += (
                "Maximum number of %s emails reached, discarded: %s. " % (hwm, p[0]))
    ...
    def __processTelephone(self, pre, p):
        ...
        if self.telephoneCount[("%s PHONE" % hwm)] <= self.maxTelephones:
            self.data["%s %s %s" % (
                hwm, telephoneType, self.telephoneCount["%s %s" % (hwm, telephoneType)])] = p[0]
            self.telephoneCount["%s %s" % (hwm, telephoneType)] += 1
        else:
            self.data['WARNING'] += (
                "Maximum number of %s telephones reached, discarded: %s. " % (hwm, p[0]))
    ...
    def __processImageUrl(self, imageType, pre, p):
        ...
        try:
            if len(pre) >= 2 and pre[1].lower() == 'value=uri'.lower():
    ...
    def __processAddress(self, pre, p):
        ...
        if self.addressCount[addressType] <= self.maxAddresses:
            self.data["%s ADDRESS %s" %
                      (addressType, self.addressCount[addressType])] = address
            self.data["%s CITY %s" %
                      (addressType, self.addressCount[addressType])] = city
            self.data["%s STATE %s" %
                      (addressType, self.addressCount[addressType])] = state
            self.data["%s ZIP %s" %
                      (addressType, self.addressCount[addressType])] = zip
            self.data["%s COUNTRY %s" %
                      (addressType, self.addressCount[addressType])] = country
            self.addressCount[addressType] += 1
        else:
            self.data['WARNING'] += (
                "Maximum number of %s addresses reached, discarded: %s. " % (addressType, p))
    ...
    def __processValueList(self, valueType, pre, p, delimiter):
        self.__trace("__processValueList: %s %s %s %s" %
                     (valueType, pre, p, delimiter))
    ...
    def __processName(self, pre, p):
        ...
        try:
            ...
            if self.data['FORMATTED NAME'] == None:
                ...
                self.__trace(
                    "_name: Invalid format for N tag, using as FN instead. %s" % p)
...
def main():
    ...
    parser = OptionParser(option_class=MyOption, usage=usa,
                          version=ver, description=des)
    ...
    parser.add_option("-o", "--output", action="store", dest="output_file",
                      default="addrs.csv", help="Name of .csv file to output too (default is addrs.csv)")
    ...
    parser.add_option("-q", "--quote", action="store_true", dest="quote",
                      default=False, help="Double quote the output strings (default is off)")
    parser.add_option("-v", "--verbose", action="store_false", dest="verbose",
                      default=True, help="Show processing information (default is on)")
    ...
    parser.add_option("-c", "--config", action="store", dest="config",
                      default=None, help="Name a config file to be used")

    (options, args) = parser.parse_args()
    if options.input_file != None and options.input_path != None:
        parser.error(
            "options -i and -p are mutually exclusive ...use one or the other but not both")

    # If we have a config file, parse it and combine into Options

    if options.config:
        cfg = os.path.normpath(os.path.expanduser(
            os.path.expandvars(options.config)))
        inp = open(cfg, 'r')
        try:
            t = inp.readlines()
        finally:
    ...
    if options.input_file != None and options.input_path == None:
        print("converting %s > %s (%s delimited)" %
              (options.input_file, options.output_file, delimiter_string))
        VcfToCsvConverter(options.input_file, options.input_path, options.output_file,
                          delimiter, options.quote, options.trace, options.verbose)
        sys.exit(0)
